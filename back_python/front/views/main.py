...
class MainView(TemplateView):
    ...
    def get(self,request,*args,**kwargs):
        ...
        if(not request.user.is_authenticated):
            ...
            fp2=None
            openid=None
            decodeUserInfoJson=None
            try:
                role=request.GET["role"];
                # 存在openid说明已经注册过了，无需再去注册，直接验证登录
                if "oid" in request.GET:
                    openid=request.GET["oid"]
                else:
                    # 检查
                    if role=="child":
                        # 重定向到学生登录页面,这个url
                    ...
                    fp1.close()
                    fp2.close()
                    openid=decodeUserInfoJson["openid"]

                # 按照openid查找用户，如果不存在就创建，存在就绕过
                userTry=None
                try:
                    userTry= User.objects.get(username__exact=openid)
                    ...
                    if role!="child":
                        # 创建新的用户信息
                        userTry=User.objects.create(username=openid)
                        #创建另一个Profile
                        # 如果role=家长，就创建一个家庭
                        orgC=None
                        if role=="host":
                            orgC=Org.objects.create(code=openid,name=decodeUserInfoJson["nickname"])
                        profile=Profile(nickname=decodeUserInfoJson["nickname"],
                        openid=openid,role=MapEngToRole[role],
                        ...
                        nkname="修改昵称"
                        userC=User.objects.create(username=openid+"@")
                        # 查询班级
                        classGroupF=None
                        #创建另一个Profile
                        profileC=Profile(nickname=nkname,role=MapEngToRole["child"],
                        imgUrl=decodeUserInfoJson["headimgurl"],user=userC,org=orgC,childpwd="987654",openid=openid)
                        profileC.save()
